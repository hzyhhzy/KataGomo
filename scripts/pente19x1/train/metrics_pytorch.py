...
class Metrics:
    ...
    def metrics_dict_batchwise_single_heads_output(
        ...
        if raw_model.config["version"] <= 11 or (raw_model.config["version"] >= 101 and raw_model.config["version"] <= 199):
            ...
            shortterm_value_excess = (shortterm_value_actual - shortterm_value_pred) #-2~2
            assert(torch.all(shortterm_value_excess<=2))
            assert(torch.all(shortterm_value_excess>=-2))
            # Or the shortterm score outcome being around 1.5 sigma more than expected
            # Add a small amount to the variance to avoid division by zero or overly small numbers
            target_weight_shortoptimistic_policy = torch.exp(shortterm_value_excess)
            #print(target_weight_shortoptimistic_policy)
            #print(target_weight_shortoptimistic_policy.mean())
        else:
            target_weight_shortoptimistic_policy = torch.zeros_like(target_weight_policy_player) + 1
        ...
        loss_policy_player = self.loss_policy_player_samplewise(
            ...
            target_weight_policy_player*target_weight_shortoptimistic_policy,
            global_weight,
        ).sum()
