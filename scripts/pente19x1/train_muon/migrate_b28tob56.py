...
def replace_block_number(varname):
    factor=1.0
    fullname=False
    ...
    match = re.search(r"blocks\.(\d+)\..*", varname)
    if match:
        #print(string)
        # 提取原始数字并增加 n
        original_number = int(match.group(1))
        #if(original_number==28):
            
        if(original_number>=28):
            original_number-=28
        # 替换字符串中的数字部分
        varname = re.sub(r"blocks\.(\d+)\.(.*)", f"blocks.{original_number}.\\2", varname)

    if fullname:
        varname="module."+varname
    return factor,varname
...
for varname in target_data["model"].keys():
    factor,varname=replace_block_number(varname)
    varname1 = "module."+varname
    if(len(varname)>7 and varname[:7]=="module."):
        varname1=varname[7:]
