...
class Metrics:
    ...
    def loss_scorebelief_cdf_samplewise(self, pred_logits, target_probs, weight, global_weight):
        ...
        loss=0.001*torch.sum(torch.square(pred_logits*0.0001),axis=1)
        #print(loss.sum())
        return loss
    ...
    def loss_scorestdev_samplewise(self, pred, scorebelief_logits, global_weight):
        ...
        loss=0.001*(torch.square(pred*0.0001)+torch.sum(torch.square(scorebelief_logits*0.0001),axis=1))
        #loss=0.001*(torch.sum(torch.square(scorebelief_logits*0.0001),axis=1))
        #print("3",loss.sum())
        return loss
