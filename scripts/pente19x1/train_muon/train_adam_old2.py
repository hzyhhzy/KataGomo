...
def main(rank: int, world_size: int, args, multi_gpu_device_ids, readpipes, writepipes, barrier):
    ...
    def save(ddp_model, swa_models, optimizer, metrics_obj, running_metrics, train_state, last_val_metrics, path=None):
    ...
    def get_weight_decay(raw_model, lr_scale, warmup_scale, train_state, running_metrics, group_name, wdscale):
        ...
        if raw_model.get_norm_kind() == "fixup" or raw_model.get_norm_kind() == "fixscale":
            if group_name == "normal" or group_name == "normal_gamma" or group_name == "output":
                return 0.00000003 * world_size * batch_size / 256.0 * wdscale
            elif group_name == "noreg":
                return 0.0000000003 * world_size * batch_size / 256.0 * wdscale
            ...
            if group_name == "normal" or group_name == "normal_gamma":
                ...
                return 0.0000375 * world_size * batch_size / 256.0 * math.pow(lr_scale_with_auto * warmup_scale,0.75) * adaptive_scale * gamma_scale * wdscale
            elif group_name == "output":
                return 0.0000003 * world_size * batch_size / 256.0 * wdscale
            elif group_name == "noreg":
                return 0.0000003 * world_size * batch_size / 256.0 * math.pow(lr_scale_with_auto * warmup_scale,0.75) * wdscale
            elif group_name == "output_noreg":
                return 0.000000003 * world_size * batch_size / 256.0 * wdscale
            else:
                assert False
        else:
            assert False
    ...
    def load():
        ...
        if path_to_load_from is None:
            ...
            optimizer = torch.optim.AdamW(get_param_groups(raw_model,train_state,running_metrics), lr=0.001, eps=1e-6)
    ...
    def update_and_return_lr_and_wd():
        per_sample_lr = 0.001*(batch_size * world_size / 256.0) * lr_scale * lr_scale_auto_factor(train_state)
        ...
        for param_group in optimizer.param_groups:
            ...
            if(param_group["eps"]<1e-7):
                logging.info("eps < 1e-7, may cause NAN for fp16 for Adam, adjusted to 1e-6")
    ...
    while True:
        ...
        for i in range(sub_epochs):
            ...
            for batch in data_processing_pytorch.read_npz_training_data(
                ...
                if model_config["norm_kind"] == "fixup" or model_config["norm_kind"] == "fixscale" or model_config["norm_kind"] == "fixscaleonenorm":
                    gnorm_cap = 7500.0 * (1.0 if gnorm_clip_scale is None else gnorm_clip_scale)
                elif model_config["norm_kind"] == "bnorm" or model_config["norm_kind"] == "brenorm" or model_config["norm_kind"] == "fixbrenorm":
                    gnorm_cap = 16500.0 * (1.0 if gnorm_clip_scale is None else gnorm_clip_scale)
                else:
                    assert False
        ...
        save(ddp_model, swa_models, optimizer, metrics_obj, running_metrics, train_state, last_val_metrics)
        ...
        if rank == 0:
            ...
            if not no_export and is_time_to_export and not skip_export_this_time and exportdir is not None and not gnorm_stats_debug:
                ...
                if os.path.exists(savepath):
                    ...
                    save(ddp_model, swa_models, optimizer, metrics_obj, running_metrics, train_state, last_val_metrics, path=os.path.join(savepathtmp,"model.ckpt"))
                    time.sleep(2)
        ...
        if rank == 0:
            ...
            if now - last_longterm_checkpoint_save_time >= datetime.timedelta(hours=12):
                ...
                save(ddp_model, swa_models, optimizer, metrics_obj, running_metrics, train_state, last_val_metrics, path=os.path.join(longterm_checkpoints_dir,f"{dated_name}.ckpt"))
