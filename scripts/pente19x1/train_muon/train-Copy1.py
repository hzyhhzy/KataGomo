...
def reset_nan_batchnorm(model, verbose=True):
    """
    检查并重置模型中所有 BatchNorm 层的 NaN/Inf 参数。
    
    参数:
        model (nn.Module): 要检查的 PyTorch 模型。
        verbose (bool): 是否打印修复信息。
    
    返回:
        bool: 是否检测到并修复了 NaN/Inf。
    """
    has_nan = False
    
    for module in model.modules():
        if True:  # 包括 BatchNorm1d/2d/3d
        #if isinstance(module, _BatchNorm):  # 包括 BatchNorm1d/2d/3d
            # 检查所有参数和缓冲区
            for name, param in module.named_parameters():
                if torch.isnan(param).any() or torch.isinf(param).any():
                    if verbose:
                        logging.info(f"重置 {name} 在 {module.__class__.__name__} (包含 NaN/Inf)")
    ...
    if verbose and not has_nan:
        logging.info("模型中没有检测到 BatchNorm 的 NaN/Inf 参数。")
