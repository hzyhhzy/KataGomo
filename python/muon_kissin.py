...
class MuonWithAuxAdamKimi(torch.optim.Optimizer):
        ...
        for group in param_groups:
            ...
            if group["use_muon"] is not None:
                if group["use_muon"]:
                    ...
                    # Adam参数组的默认值
                    group["betas"] = group.get("betas", (0.9,  self.momentum_default))
                    group["eps"] = group.get("eps", 1e-8)
                    group["weight_decay"] = group.get("weight_decay", 0)
    ...
    def _auto_detect_muon(self, param, group_name: str) -> bool:
        ...
        if "normal" in group_name.lower():
            assert(group_name=="normal" or group_name=="normal_attn" )
    ...
    def step(self):
        for group in self.param_groups:
            ...
            if group.get("use_muon", None) is None:
                ...
                for base_i in range(len(params))[::dist.get_world_size() if self.is_distributed else 1]:
                    if not self.is_distributed or base_i + dist.get_rank() < len(params):
                        ...
                        if "momentum_buffer" not in state:
                            state["momentum_buffer"] = torch.zeros_like(p)
                        update = muon_update_kimi(p.grad, state["momentum_buffer"], beta=group.get("momentum",  self.momentum_default))
                ...
                # Adam参数处理 (使用原始学习率)
                beta1, beta2 = group.get("betas", (0.9,  self.momentum_default))
                for p in group["params"]:
                    if p.grad is None:
                        continue
                    state = self.state[p]
                    if not state["use_muon"]:
                        ...
                        state["step"] += 1
                        update = adam_update(p.grad, state["exp_avg"], state["exp_avg_sq"],
                                           state["step"], group.get("betas", (0.9,  self.momentum_default)), group.get("eps", 1e-10))
                        p.mul_(1 - adam_lr * group.get("weight_decay", 0))
