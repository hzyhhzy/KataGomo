...
def main(rank: int, world_size: int, args, multi_gpu_device_ids, readpipes, writepipes, barrier):
    ...
    def pslr_func(batchsize,lrscale):
        pslr = 3e-6 * lrscale * (batchsize/1.0)**padamw_p
        return pslr
        ...
        if raw_model.get_norm_kind() == "fixup" or raw_model.get_norm_kind() == "fixscale":
            ...
            if group_name == "normal" or group_name == "normal_gamma" or group_name == "normal_attn":
                ...
                #    ratio = norm_normal_batch / (baseline + 1e-30)
                    ...
                    #logging.info(f"{running_metrics["weights"]["norm_normal_batch"]},{norm_normal_batch},{baseline},{adaptive_scale}")
                ...
                #return 1.0 * wd0 * adaptive_scale * gamma_scale * wdscale
                return 1.0 * wd0 * 4.0 * gamma_scale * wdscale
            elif group_name == "output":
                return 1.0 * wd0 * wdscale
            elif group_name == "noreg":
                return 0.01 * wd0 * wdscale
            elif group_name == "output_noreg":
                return 0.001 * wd0 * wdscale
            else:
                assert False
        else:
            assert False
    ...
    def get_param_groups(raw_model,train_state,running_metrics):
        ...
        if len(reg_dict["normal_attn"]) > 0:
            param_groups.append({
                ...
                "weight_decay": get_weight_decay(raw_model, lr_scale, warmup_scale=1.0, train_state=train_state, running_metrics=running_metrics, group_name="normal_attn", wdscale=wdscale),
