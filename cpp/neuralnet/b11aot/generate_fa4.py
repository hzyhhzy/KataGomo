...
"""Standalone CPU-only SM120 FA4 AOT: S361/H12/D32, FP16 QK/PV, runtime batch."""
import argparse
import hashlib
...
def parse_args():
    ...
    args.symbol_prefix = args.symbol_prefix or (
        f"b11_fa4_{args.mode}_m{args.tile_m}_n{args.tile_n}_s{args.num_stages}")
...
// Every tensor must be 16-byte aligned; batch is runtime, 1..{args.max_batch}.
...
extern "C" cudaError_t {p}_launch(void* q,void* k,void* v,void* output,
  void* mask,int batch,float scale,bool packedQKV,cudaStream_t stream) {{
  if(batch<1 || batch>{args.max_batch} || !q || !k || !v || !output || {mask_guard})
  ...
  constexpr int seq=361,heads=12,dim=32;
...
{mask_tensor}  int32_t status=cute_dsl_{p}_wrapper(loader.module(),&tq,&tk,&tv,&to,{mask_arg}scale,stream);
...
def compile_artifact(args, dependencies):
    ...
    from fake_tensors import dynamic_fp16_tensor
    ...
    q,k,v,o = [dynamic_fp16_tensor(
        (batch,seq,heads,dim),(seq*heads*dim,heads*dim,dim,1),address,3)
        for address in (0x10000,0x20000,0x30000,0x40000)]
    scale = 1.0/(dim**0.5)
    stream = cute.runtime.make_fake_stream(use_tvm_ffi_env_stream=False)
    if masked:
        mask = dynamic_fp16_tensor((batch,seq),(seq,1),0x50000,1)
        @cute.jit
...
def main():
    ...
    nvcc = subprocess.run([str(args.cuda_root / "bin/nvcc"),"--version"],
                          check=True,text=True,capture_output=True).stdout
    metadata = {
        "schema":1,"mode":args.mode,"symbol_prefix":p,
        "prepare_symbol":p+"_prepare","launch_symbol":p+"_launch",
        "fixed_shape":{"sequence":361,"heads":12,"head_dim":32},
        "runtime_batch":{"minimum":1,"maximum":args.max_batch,"probe":args.probe_batch},
