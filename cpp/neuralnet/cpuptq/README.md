...
The v106 grammar preserves v102's optional QKN, arbitrary
...
Version 106 keeps the floating v102 body and its `@V102_QKN_CLIP@` extension,
but replaces every
...
CPU v106 contains none of CUDA v105's `attentionInputQuantMaxAbs`,
`attentionOutputQuantMaxAbs`, `ffnInputQuantMaxAbs`, or `productQuantMaxAbs`.
CPU activation scales are computed dynamically at runtime. The shared C++
descriptor retains those members solely for CUDA v105; they remain zero for
v102/v106. The retired v105-body v106 dialect is intentionally rejected.
Re-export old artifacts from the checkpoint or floating v102 base; do not
just change a v105 model's version number.

All maintained Python export/calibration code lives in `KataGo_Transformer`.
From that repository, convert a floating native v102 model without overwriting
the source:

```text
python train/export_cpu_ptq.py --source model-v102.bin.gz --output runtime_data/model-s8-v106.bin.gz --cpu-quantization s8
python train/export_cpu_ptq.py --source model-v102.bin.gz --output runtime_data/model-s7-v106.bin.gz --cpu-quantization s7
```
...
python train/calibrate_cpu_ptq.py \
  ...
  --output runtime_data/model-s8-gptq.npz --qmax 127 --board-size 15
python train/export_cpu_ptq.py \
  --checkpoint checkpoint.ckpt --model-name cpu-b16 --pos-len 15 \
  --output runtime_data/model-s8-v106.bin.gz --cpu-quantization s8 \
  --manifest runtime_data/model-s8-gptq.npz

python train/calibrate_cpu_ptq.py \
  --checkpoint checkpoint.ckpt --data calibration.npz \
  --output runtime_data/model-s7-gptq.npz --qmax 63 --board-size 15
python train/export_cpu_ptq.py \
  --checkpoint checkpoint.ckpt --model-name cpu-b16 --pos-len 15 \
  --output runtime_data/model-s7-v106.bin.gz --cpu-quantization s7 \
...
The unified converter uses v102 as the v106 source and v11 as the Ataxx v206
source. It rejects CUDA v105/v205 input, validates projection geometry and
source hashes, replaces all seven matrices per layer, and writes deterministic
gzip (`mtime=0`) through an atomic temporary file. The engine validates the
complete native body and the compiled profile when loading.
...
The calibration and converter contract tests run from `KataGo_Transformer`:

```text
python -m unittest train.tests.test_cpu_ptq_v106 -v
```

The C++ `testv105wire` command also tests v106 S7/S8 projection storage,
QKN on/off, clip 0/4/7, fixed/learned/no RoPE, and rejection of the retired
v105-shell encoding while preserving the CUDA-v105 contract. To check a real
v106 file without running the inference kernel, use
`katago.exe testv105wire --model-v106 MODEL.bin.gz SHA256`.
