      ...
      for(int i = 0; i<numBoards; i++) {
        ...
        if(!suc[i]) {
          if(board.isOnBoard(loc)) {
            ...
            testAssert(loc < 0 || loc >= Board::MAX_ARR_SIZE || board.colors[loc] != C_EMPTY || board.isIllegalSuicide(loc,pla,multiStoneSuicideLegal[i]) || board.isKoBanned(loc,0));
            if(board.isKoBanned(loc,0)) {
