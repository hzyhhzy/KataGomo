...
#define FOREACHADJ(BLOCK) {int ADJOFFSET = -(x_size+1); {BLOCK}; ADJOFFSET = -1; {BLOCK}; ADJOFFSET = 1; {BLOCK}; ADJOFFSET = x_size+1; {BLOCK}; ADJOFFSET = x_size; {BLOCK}; ADJOFFSET = -x_size; {BLOCK}};
...
void Board::calculateAreaForPla(
  ...
  //independent regions, each one vital for at most 6 pla groups, add some extra just in case.
  static constexpr int maxRegions = (MAX_LEN * MAX_LEN + 1)/2 + 1;
  static constexpr int vitalForPlaHeadsListsMaxLen = maxRegions * 6;
  ...
  for(int y = 0; y < y_size; y++) {
    for(int x = 0; x < x_size; x++) {
      ...
      {
        uint16_t vStart = vitalStart[regionIdx];
        assert(vStart + 6 <= vitalForPlaHeadsListsMaxLen);
      ...
      if(basicArea[loc] != C_EMPTY && !isSeki[loc]) {
        if(
          ...
          ((colors[loc+ADJ0] == C_EMPTY && basicArea[loc+ADJ0] == C_EMPTY) ||
           ...
           (colors[loc+ADJ4] == C_EMPTY && basicArea[loc+ADJ4] == C_EMPTY) ||
           (colors[loc+ADJ5] == C_EMPTY && basicArea[loc+ADJ5] == C_EMPTY))
        ) {
