#!/usr/bin/env python3
"""CPU-only exact C256/H8/S361/D32 SM120 FlashAttention-4 generator.
...
``c256s361fa4win`` ABI instead of rewriting an object file.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--num-threads", type=int, choices=(32, 64, 128, 256), default=128)
...
def candidate_id(args: argparse.Namespace) -> str:
    ...
    return (
        ...
        f"{args.accumulation}-{args.layout}-qregs{int(args.q_in_regs)}-b{args.batch}-"
...
def abi(args: argparse.Namespace) -> tuple[str, str]:
    if args.abi_mode == "production":
        return "c256_s361_fa4_winner", "c256s361fa4win"
    warps = args.num_threads // 32
    layout = "p" if args.layout == "planar" else "t"
    suffix = (
        f"b{args.batch}m{args.tile_m}n{args.tile_n}s{args.num_stages}"
        f"w{warps}qr{int(args.q_in_regs)}{args.accumulation}{layout}"
    )
    return f"c256_s361_fa4_{suffix}", f"c256s361fa4{suffix}"
...
def generate(args: argparse.Namespace) -> Path:
    ...
    fa = FlashAttentionForwardSm120(
        ...
        Q_in_regs=args.q_in_regs, score_mod=None, mask_mod=None,
        has_aux_tensors=False,
    )
    fa.qk_acc_dtype = qk_dtype
    fa.pv_acc_dtype = pv_dtype
